...
DynamixelController::DynamixelController()
    : Node("dynamixel_controller_node"), port_handler_(nullptr),
  ...
  if (!port_handler_) {
    RCLCPP_ERROR(this->get_logger(), "Failed to create port handler for: %s",
                 DEVICE_NAME);
    rclcpp::shutdown();
    return;
  }
  ...
  if (!packet_handler_) {
    RCLCPP_ERROR(this->get_logger(),
                 "Failed to create packet handler for protocol version: %.1f",
                 protocol_version_);
    if (port_handler_) {
      port_handler_->closePort();
    }
  ...
  instruction_subscriber_ =
      this->create_subscription<dynamixel_controller::msg::DynamixelCommand>(
          "dynamixel_tx", 100,
          std::bind(&DynamixelController::instruction_callback, this,
                    std::placeholders::_1));

  // ROS2 パブリッシャーの作成 (受信応答を publish する)
  response_publisher_ =
      this->create_publisher<dynamixel_controller::msg::DynamixelResponse>(
          "dynamixel_rx", 10);
}
...
void DynamixelController::publish_response(uint8_t instruction_code,
...
void DynamixelController::instruction_callback(
    const dynamixel_controller::msg::DynamixelCommand::SharedPtr msg) {
  switch (msg->command) {
  case MSG::READ_DATA: {
  ...
  default:
    RCLCPP_WARN(this->get_logger(), "Received unsupported command: %d",
                msg->command);
    break;
  }
}

void DynamixelController::handle_read_command(
    const dynamixel_controller::msg::DynamixelCommand::SharedPtr msg) {
  ...
  if (!port_handler_ || !packet_handler_) {
    RCLCPP_ERROR(this->get_logger(),
                 "Port handler or packet handler not initialized");
    return;
  }
  ...
  if (read_length == 1) {
    ...
    comm_result = packet_handler_->read1ByteTxRx(
        port_handler_, dxl_id, read_address, &data, &dxl_error);
  ...
  if (comm_result != COMM_SUCCESS) {
    RCLCPP_ERROR(this->get_logger(), "Read failed for ID %d: %s", dxl_id,
    ...
    RCLCPP_INFO(this->get_logger(),
                "READ_DATA success - ID: %d, Address: %d, Length: %d, Data: "
                "[%s], Error: %d",
...
void DynamixelController::handle_write_command(
    const dynamixel_controller::msg::DynamixelCommand::SharedPtr msg) {
  ...
  if (data_length == 1) {
    if (msg->data.size() < 1) {
      RCLCPP_ERROR(this->get_logger(),
                   "Insufficient data for 1-byte write. Expected: 1, Got: %zu",
                   msg->data.size());
      return;
    }
    uint8_t value = msg->data[0];
    comm_result = packet_handler_->write1ByteTxRx(
        port_handler_, dxl_id, write_address, value, &dxl_error);
  } else if (data_length == 2) {
    if (msg->data.size() < 2) {
      RCLCPP_ERROR(this->get_logger(),
                   "Insufficient data for 2-byte write. Expected: 2, Got: %zu",
                   msg->data.size());
      return;
    }
    ...
    if (msg->data.size() < 4) {
      RCLCPP_ERROR(this->get_logger(),
                   "Insufficient data for 4-byte write. Expected: 4, Got: %zu",
                   msg->data.size());
      return;
    }
    uint32_t value = msg->data[0] | (msg->data[1] << 8) | (msg->data[2] << 16) |
                     (msg->data[3] << 24);
    comm_result = packet_handler_->write4ByteTxRx(
        port_handler_, dxl_id, write_address, value, &dxl_error);
  } else {
    RCLCPP_ERROR(this->get_logger(),
                 "Unsupported data length: %d. Supported: 1, 2, 4 bytes",
                 data_length);
    return;
  }
  ...
  if (comm_result != COMM_SUCCESS) {
    RCLCPP_ERROR(this->get_logger(), "Write failed for ID %d: %s", dxl_id,
                 packet_handler_->getTxRxResult(comm_result));
    errors[0] = 255; // Communication error
  } else {
    ...
    RCLCPP_INFO(this->get_logger(),
                "WRITE_DATA success - ID: %d, Address: %d, Length: %d, Data: "
                "[%s], Error: %d",
                dxl_id, write_address, data_length, data_str.c_str(),
                dxl_error);
  }
...
void DynamixelController::handle_sync_read_command(
    const dynamixel_controller::msg::DynamixelCommand::SharedPtr msg) {
  if (!msg || msg->ids.empty()) {
    RCLCPP_ERROR(this->get_logger(),
                 "SYNC_READ command requires at least one ID");
    return;
  }
  ...
  if (!ttl_targets.empty()) {
    dynamixel::GroupSyncRead ttlRead(port_handler_, packet_handler_,
    ...
    if (comm_result == COMM_SUCCESS) {
      for (auto id : ttl_targets) {
        if (ttlRead.isAvailable(id, start_address, data_length)) {
          ...
          RCLCPP_INFO(this->get_logger(),
                      "SYNC_READ success (TTL) - ID: %d, Address: %d, Length: "
                      "%d, Data: [%s]",
                      id, start_address, data_length, data_str.c_str());
        }
      }
    } else {
      RCLCPP_ERROR(this->get_logger(), "SYNC_READ failed for TTL devices: %s",
                   packet_handler_->getTxRxResult(comm_result));
    }
  }
  ...
  if (!rs_targets.empty()) {
    dynamixel::GroupSyncRead rsRead(port_handler_, packet_handler_,
                                    start_address, data_length);
    ...
    if (comm_result == COMM_SUCCESS) {
      for (auto id : rs_targets) {
        if (rsRead.isAvailable(id, start_address, data_length)) {
          ...
          RCLCPP_INFO(this->get_logger(),
                      "SYNC_READ success (RS485) - ID: %d, Address: %d, "
                      "Length: %d, Data: [%s]",
                      id, start_address, data_length, data_str.c_str());
        }
      }
    } else {
      RCLCPP_ERROR(this->get_logger(), "SYNC_READ failed for RS485 devices: %s",
                   packet_handler_->getTxRxResult(comm_result));
    }
  }

  publish_response(MSG::SYNC_READ, response_ids, response_errors,
                   response_data);
}

void DynamixelController::handle_sync_write_command(
    const dynamixel_controller::msg::DynamixelCommand::SharedPtr msg) {
  if (!msg || msg->ids.empty() || msg->data.empty()) {
    RCLCPP_ERROR(this->get_logger(),
                 "SYNC_WRITE command requires IDs and data");
    return;
  }
  ...
  if (msg->data.size() != expected_data_size) {
    RCLCPP_ERROR(this->get_logger(),
                 "SYNC_WRITE: data size mismatch. Expected %zu bytes, got %zu",
  ...
  if (data_length == 0 || data_length > 4) {
    RCLCPP_ERROR(this->get_logger(),
                 "SYNC_WRITE: invalid data length %d. Supported: 1-4 bytes",
                 data_length);
    return;
  }

  dynamixel::GroupSyncWrite ttlWrite(port_handler_, packet_handler_,
                                     start_address, data_length);
  dynamixel::GroupSyncWrite rsWrite(port_handler_, packet_handler_,
                                    start_address, data_length);
  ...
  for (size_t i = 0; i < msg->ids.size(); i++) {
    ...
    for (uint16_t j = 0; j < data_length; j++) {
      ...
      if (data_index >= msg->data.size()) {
        RCLCPP_ERROR(
            this->get_logger(),
            "SYNC_WRITE: data index out of bounds. Index: %zu, Size: %zu",
  ...
  if (ttl_has_param) {
    ...
    if (comm_result == COMM_SUCCESS) {
      ...
      RCLCPP_INFO(
          this->get_logger(),
          "SYNC_WRITE success (TTL) - IDs: [%s], Address: %d, Length: %d",
          id_str.c_str(), start_address, data_length);
    } else {
      RCLCPP_ERROR(this->get_logger(), "SYNC_WRITE failed for TTL devices: %s",
                   packet_handler_->getTxRxResult(comm_result));
    }
  }
  ...
  if (rs_has_param) {
    ...
    if (comm_result == COMM_SUCCESS) {
      ...
      RCLCPP_INFO(
          this->get_logger(),
          "SYNC_WRITE success (RS485) - IDs: [%s], Address: %d, Length: %d",
          id_str.c_str(), start_address, data_length);
    } else {
      RCLCPP_ERROR(this->get_logger(),
                   "SYNC_WRITE failed for RS485 devices: %s",
                   packet_handler_->getTxRxResult(comm_result));
    }
  }
  ...
  publish_response(MSG::SYNC_WRITE, response_ids, response_errors,
                   response_data);
}

void DynamixelController::handle_ping_command(
    const dynamixel_controller::msg::DynamixelCommand::SharedPtr msg) {
  if (!msg) {
  ...
  for (auto id : msg->ids) {
    ...
    if (comm_result == COMM_SUCCESS) {
      ...
      RCLCPP_INFO(this->get_logger(), "PING success - ID: %d, Error: %d", id,
                  dxl_error);
    } else {
      response_errors.push_back(255); // Communication error
      RCLCPP_WARN(this->get_logger(), "Ping failed for ID %d: %s", id,
                  packet_handler_->getTxRxResult(comm_result));
    }
  }
