...
`rtz_to_joint_trajectory` は `geometry_msgs/msg/Vector3` を購読します。
テスト用に `/rtz_cmd` を手動で送る例は以下です。

```bash
# 1回だけ送信
ros2 topic pub --once /rtz_cmd geometry_msgs/msg/Vector3 "{x: 0.1, y: 0.2, z: 0.3}"
```

```bash
# 10 Hz で連続送信
ros2 topic pub -r 10 /rtz_cmd geometry_msgs/msg/Vector3 "{x: 0.1, y: 0.2, z: 0.3}"
```
